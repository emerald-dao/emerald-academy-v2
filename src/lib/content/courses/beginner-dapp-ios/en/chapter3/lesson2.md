...
# Chapter 3 Lesson 2 - Running a Script
...
We learned in Chapter 3 Lesson 1 (yesterday) that we can do this by using `useEffect` to do something every time something happens (which in this case is a page refresh). Let's do that in code by doing two things:
...
But how do we make sure this happens every time we refresh the page? Remember from Chapter 3 Lesson 1 that you have to add the `[]`:
