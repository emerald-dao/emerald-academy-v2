...
When storing resources to our account, it's annoying (and very inconveniant) to store every single resource to a different storage path.

Imagine if you purchased 100 NFTs and had to store each one separately, and then remember where they were so you could access them in the future. Not good.

Instead, we can define a wrapper resource (ex. "Collection") that we can store at 1 storage path, and which itself stores nested resources ("NFT"s).
